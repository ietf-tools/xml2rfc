...
proc pass2begin_rfc {elemX} {
    ...
    if {[string compare $attrs(number) ""]} {
        ...
        if {   [string first "3667" $attrs(ipr)] >= 0
            ...
            # TLP 2009 09 copyright notice beginning with Dec 2009
            set newP 7
        }
    ...
    if {$newP == 8} {
        ...
        # IDs: include in Status Of This Memo (produced elsewhere), for date before Dec 2009
        if {![string compare $attrs(ipr) "pre5378Trust200902"] && \
            ([string compare $attrs(number) ""] || $ymd >= "20091201")} {
