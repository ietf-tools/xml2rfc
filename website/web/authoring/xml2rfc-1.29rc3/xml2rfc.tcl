...
proc prexml {stream} {
    ...
    set stream [prexml_cdata [str_norm_eol $stream]]
    ...
    if {   [regexp -- {^<\?xml[ \t\r\n]} $stream]
        ...
        set prolog [string range $stream 0 [incr plen]]
        set stream [string range $stream [incr plen] end]
    } else {
        error "input file \"$ifile\" does not begin with an XML prolog"
    }

    linefile::init

    return $prolog[prexml_stream $stream $ifile \
                                 [expr 1 + [num_eols $prolog]]]
}
...
proc prexml_stream {stream myfile {myline 1} {no_initial_pi 0}} {
                ...
                while {1} {
                    ...
                    if {![string compare $key ""]} {
                        ...
                        if {   [string compare $val ""]
                            ...
                            error "stopped preprocessing faulty rfc-PI at \"$val\""
                        }
                        break
                    }
                    switch -- $key {
  ...
  "ignoring invalid rfc-PI linefile-directive value \"$val\" around input line $myline in \"$myfile\""
                            ...
                            set myline $b
                            # This new myline value is valid at the end of the PI.
                            set new_lines 0
                            continue
                        }
                    ...
                    # Go on with include directive.
                    set pi [streplace $pi $pi2s [expr $pi2l - [string length $pi2] + 1] "" 3]
                    set l [prexml_find_file $val]
                    set file    [lindex $l 0]
                    set include [lindex $l 1]

                    set include [prexml_tweak_include $include]
                    if {[string compare $include ""]} {
                        append includes [prexml_stream $include $file 1]
                    }
                }
                #if {![string compare $pi ""]} {
                #    # Although legal XML, our own SGML parser could
                #    # not handle it in the past, so just to be sure.
                #    set pi " "
                #}
                if {$kept_pi_dirs} {
                    prexml_flush_blank
            ...
            <!ENTITY {
                ...
                if {![regexp -nocase -- \
                    ...
                    error "expecting $m $entity SYSTEM, PUBLIC, 'def', or \"def\""
                }
                if {   ![regexp -- ^$wsp+$dqev$rem $entid x arg1 arg2]
                    && ![regexp -- ^$wsp+$sqev$rem $entid x arg1 arg2]} {
                    error "expecting literal after $m $entity $idtype"
                }
...
proc prexml_find_file {f} {
    ...
    foreach dir $xml_library_path {
        ...
        set content [prexml_cdata [str_norm_eol [read $fd]]]
        catch { close $fd }
        set foundP 1
        break
    }
...
proc prexml_find_uri {u} {
    ...
    set content [prexml_cdata [str_norm_eol [http::data $httpT]]]
    http::cleanup $httpT
...
proc prexml_tweak_include {include} {
...
proc around2fl {result} {
    ...
    if {[regexp -nocase -- { around line ([1-9][0-9]*)} \
                $result x lineno] != 1} {
        return $result
    }
            ...
            while {1} {
                ...
                switch -- $key {
                    ...
                    linefile {
                        ...
                        linefile::set_line [expr $b - $lines]
                        continue
                    }
                }
...
proc pi_rfc_next {text} {
    ...
    if {   [regexp -- ^$nme=$dqev$rem$ $text x k v rest]
        ...
        || [regexp -- ^$nme=${nme}(|$wsp.*)$ $text x k v rest]} {
        set key $k
...
proc unexpected {args} {
    ...
    switch -- [set type [lindex $args 0]] {
        ...
        default {
            ...
            if {$::xdv::lineno >= 0} {
                append text [around2fl " around line $::xdv::lineno"]
            }
...
proc chars_expand {text {flatten 1}} {
    ...
    if {[regexp -- {&#1(2[89]|[345][0-9]);} $text]} {
        unexpected [unex_condtype] \
            "use of entities between &#128; and &#159; as printable characters is non-standard and should be avoided"
    }
