...
<<COMMENT;
########################### #######
 Process an check-svg HTTP request

 invoking form has these fields:
	file "input" or "url"
	    contains the SVG file
	radio button "type"
...
my $newinputfn = setSubTempFile("$basename.svg");
...
####### ########################### #######
####### #
####### # Run check-svg to: expand internal references and includes.
####### #
####### ########################### #######

my $curfile = $newinputfn;

my $tmpout = callCheckSvg("svg");
$curfile = $tmpout;
...
if ($type eq 'towindow') {
    ...
    print "<html><head><title>CHECK-SVG Processor with Warnings &amp; Errors</title></head>\n";
...
sub callCheckSvg {
    ...
    my ($ret, $out, $err) = runCommand("svgcheck --cache=/tmp/svgcheck -o $tmpout $curfile", $curfile, $tmpout, "Running check-svg");
